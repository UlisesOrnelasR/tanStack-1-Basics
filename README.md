...
| 🗺️ | [Routing](#️-routing) |
| 🏷️ | [Metadata & Head Management](#️-metadata--head-management) |
...
## 🏷️ Metadata & Head Management

TanStack Router has **built-in head management** via the `head()` option. No external library needed — just define what goes in `<head>` right where the route is declared.

> 💡 **The key insight**: metadata is a routing concern, not a component concern. Each route declares its own `<title>`, `<meta>`, and `<link>` tags — and they compose automatically from root to leaf.

---

### 🌐 Global Metadata — `__root.tsx`

Set the baseline metadata for the entire app in the root route. Every page inherits these unless a child route overrides them.
...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      { title: "Tanstack Basics" },
      { name: "description", content: "Discover and share top development skills." },
    ],
    links: [
...
| Field | What it does |
|-------|-------------|
| `charSet` | Declares `<meta charset="utf-8">` |
| `name: "viewport"` | Controls mobile scaling |
| `title` | Default `<title>` for all pages |
| `name: "description"` | Default meta description for SEO |
| `links` | Global `<link>` tags (stylesheets, fonts, icons) |

---

### 📄 Per-Route Metadata — `head()` in `createFileRoute`

Any route can define its own `head()` to override the global defaults. The router **merges** them — root first, then the leaf route on top.
...
export const Route = createFileRoute("/about")({
  ...
  head: () => ({
    meta: [
      { title: "About Us" },                                    // 👈 overrides root title
      { name: "description", content: "About us............." }, // 👈 overrides root description
    ],
  }),
});
```
...
🌐 Browser navigates to /about
         │
         ▼
  __root head() runs   →  charSet, viewport, default title, global CSS
         │
         ▼
  /about head() runs   →  "About Us" title, custom description
         │
         ▼
  <head> = merged result (leaf wins on conflicts)
```

---

### 🗺️ Metadata Inheritance at a Glance

| Route | `<title>` | `<meta name="description">` |
|-------|-----------|-----------------------------|
| `/` (root fallback) | `Tanstack Basics` | `Discover and share top development skills.` |
| `/about` | `About Us` | `About us.............` |
| Any other route | `Tanstack Basics` (inherited) | root description (inherited) |

> ✅ **No flash, no layout shift** — `<HeadContent />` is rendered inside `<head>` synchronously on the server, so the browser never sees a wrong title.
